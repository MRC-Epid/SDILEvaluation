...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2=="MilkBased"]
...
###############################################################################
#CONVERT VOLUME TO 100ML
###############################################################################

#SDIL$volume <- ifelse(SDIL$SDIL2=="MilkBased",(SDIL$volume/100),SDIL$volume)
...
MilkBased <- gls(volume ~ Time + Indicator + IndicatorTime + Level + Trend + 
                IndicatorLevel + IndicatorTrend + AverageMonthlyTemperature + December +
                January, data=SDIL,correlation=corARMA(p=15,form=~Time|Indicator),method="ML")
