...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("HigherTier","Toiletries")]
...
# summary(update(model_ols, ~.+ I(AnnouncementIndicatorTrend^2) + I(AnnouncementIndicatorTrend^3))) #YES  - BUT DON'T USE FOR CONSISTENCY WITH PREVIOUS ANALYSIS
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=11,form=~Time|Indicator)))
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=18,form=~Time|Indicator)))
...
HigherTier <- gls(Model, data=SDIL, correlation=corARMA(p=2, q=2, form=~Time|Indicator),method="ML")
