...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("NoSugar","Toiletries")]
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=18,form=~Time|Indicator)))
# 
# ###############################################################################
...
NoSugar <- gls(Model, data=SDIL, correlation=corARMA(p=4, q=4, form=~Time|Indicator),method="ML")
