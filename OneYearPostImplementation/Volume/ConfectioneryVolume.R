...
# 
###############################################################################
#ADD ORDER TERMS - LOOKS LIKE OVER FITTING SO NO TERMS INCLUDED
###############################################################################
# 
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=13,form=~Time|Indicator))) #NON CONVERGENCE ERROR 
# 
# ###############################################################################
...
Confectionery <- gls(Model, data=SDIL, correlation=corARMA(p=4,form=~Time|Indicator),method="ML")
