...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("Alcohol","Toiletries")]
...
# 
# model_ols <- summary(update(model_ols, ~. +I(ImplementationIndicatorTrend^2) + I(ImplementationIndicatorTrend^3) + I(ImplementationIndicatorTrend^4)))
# 
###############################################################################
#UPDATE FORMULA
###############################################################################
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=11,form=~Time|Indicator)))
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=15,form=~Time|Indicator)))
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=20,form=~Time|Indicator)))
...
Alcohol <- gls(Model, data=SDIL, method="ML")
