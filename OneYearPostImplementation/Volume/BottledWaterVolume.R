...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("BottledWater","Toiletries")]
...
# summary(update(model_ols, ~.+ I(AnnouncementIndicatorTrend^2) + I(AnnouncementIndicatorTrend^3) + I(AnnouncementIndicatorTrend^4))) #YES - BUT DON'T USE FOR CONSISTENCY WITH PREVIOUS ANALYSIS
# 
...
# 
# model_ols <- summary(update(model_ols, ~. +I(AnnouncementIndicatorTrend^2) + I(AnnouncementIndicatorTrend^3) + 
#                               I(ImplementationIndicatorTrend^2) + I(ImplementationIndicatorTrend^3)))
# 
###############################################################################
#UPDATE FORMULA
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=14,form=~Time|Indicator)))
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=16,form=~Time|Indicator))) 
# anova(ARIMA,update(ARIMA,correlation=corARMA(q=26,form=~Time|Indicator))) #NON CONVERGENCE ERROR
# 
# ###############################################################################
# #CONTINUE CORRELATION STRUCTURE CHECKS - P AND Q
# ###############################################################################
# 
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=2, q=2,form=~Time|Indicator))) #COEFFICIENT MATRIX ERROR
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=3, q=3,form=~Time|Indicator))) #COEFFICIENT MATRIX ERROR
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=16, q=16,form=~Time|Indicator))) #NON CONVERGENCE ERROR

###############################################################################
#UPDATE TO MODEL OF BEST FIT
###############################################################################

BottledWater <- gls(Model, data=SDIL, correlation=corARMA(p=26,form=~Time|Indicator),method="ML")
