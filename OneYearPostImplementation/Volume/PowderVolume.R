...
if(!exists("WeeklyVolume")){
  source("/dph/sdil/KWP221/ITSData19.R")
}
...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("Powder","Toiletries")]
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=5, q=5,form=~Time|Indicator))) #NON CONVERGENCE ERROR
...
Powder <- gls(Model, data=SDIL, correlation=corARMA(p=8,form=~Time|Indicator),method="ML")
