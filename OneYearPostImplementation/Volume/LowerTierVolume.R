...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("LowerTier","Toiletries")]
...
###############################################################################
# 
# summary(update(model_ols, ~.+ I(AnnouncementIndicatorTrend^2))) # YES
# summary(update(model_ols, ~.+ I(AnnouncementIndicatorTrend^2) + I(AnnouncementIndicatorTrend^3))) #YES - BUT DONT INCLUDE FOR CONSISTENCY
...
# 
# 
# summary(update(model_ols, ~.+ I(AnnouncementIndicatorTrend^2) + I(ImplementationIndicatorTrend^2))) #YES
...
Model <- as.formula(volume ~ Time + SDILAnnouncement + SDILImplementation + 
                      ...
                      I(AnnouncementIndicatorTrend^2))
...
#CHOSE Q=4 AS THE CONFIDENCE INTERVALS IN THE RELATIVE AND ABSOLUTE CHANGES ARE THE SAME
#THIS IS NOT THE CASE IF 4 IS CHOSEN
###############################################################################

LowerTier <- gls(Model, data=SDIL, correlation=corARMA(p=16,form=~Time|Indicator),method="ML")
