###############################################################################
#REGRESSION TABLE WITHOUT A CONTROL GROUP
#DAVID PELL
#27/04/2020
###############################################################################
...
#TIDY UP
###############################################################################
#TIDY UP TO MAKE SURE THERE ARE NO MODELS ALREADY IN ENVIRONMENT
###############################################################################
...
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/HigherTierUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/LowerTierUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/NoLevyUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/LowSugarUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/NoSugarUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/BottledWaterUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/AlcoholUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/MilkBasedUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/NASFruitJuiceUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/PowderUncontrolledVolume.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Volume/ConfectioneryUncontrolledVolume.R")

###############################################################################
#RUN REGRESSION MODELS - SUGAR
###############################################################################

source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/HigherTierUncontrolledSugar.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/LowerTierUncontrolledSugar.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/NoLevyUncontrolledSugar.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/LowSugarUncontrolledSugar.R")      
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/MilkBasedUncontrolledSugar.R")  
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/NASFruitJuiceUncontrolledSugar.R")
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/PowderUncontrolledSugar.R")  
source("/dph/sdil/KWP221/Analysis/SensitivityAnalysis/UncontrolledAnalysis/Sugar/ConfectioneryUncontrolledSugar.R")

###############################################################################
#RUN ABSOLUTE DIFFERENCES FUNCTIONS
###############################################################################

source("/dph/sdil/KWP221/Analysis/RegressionTable/Uncontrolled/AbsoluteDifferencesUncontrolledFunctions19.R")
...
source("/dph/sdil/KWP221/Analysis/RegressionTable/Uncontrolled/AbsoluteDifferencesImplementationUncontrolledLowerTier.R")

###############################################################################
#RUN ABSOLUTE DIFFERENCE FUNCTIONS FOR LOWER TIER ANNOUNCEMENT AND IMPLEMENTATION
###############################################################################

source("/dph/sdil/KWP221/Analysis/RegressionTable/Uncontrolled/AbsoluteDifferencesAnnouncementImplementationUncontrolledLowerTier.R")

###############################################################################
#RUN RELATIVE DIFFERENCES POST ANNOUNCEMENT FUNCTIONS
###############################################################################

source("/dph/sdil/KWP221/Analysis/RegressionTable/Uncontrolled/RelativeDifferencesFunctionsUncontrolledAnnouncement19.R")

###############################################################################
#RUN RELATIVE DIFFERENCES PRE-ANNOUNCEMENT TO POST IMPLEMENTATION FUNCTIONS
###############################################################################

source("/dph/sdil/KWP221/Analysis/RegressionTable/Uncontrolled/RelativeDifferencesFunctionsUncontrolledAnnouncementImplementation19.R")

###############################################################################
#RUN RELATIVE DIFFERENCES POST IMPLEMENTATION FUNCTIONS
###############################################################################

source("/dph/sdil/KWP221/Analysis/RegressionTable/Uncontrolled/RelativeDifferencesFunctionsUncontrolledImplementation19.R")
...
ValueA.fun <- function(model){
  ...
  ifelse(sum(names(coef(model.object)) %in% "I(AnnouncementIndicatorTrend^2)")==1,
         ...
         summary(model.object)$tTable["AnnouncementTrend","Value"])
}
...
LowerCIA.fun <- function(model){
  ...
  ifelse(sum(names(coef(model.object)) %in% "I(AnnouncementIndicatorTrend^2)")==1,
         ...
         confint(model.object)["AnnouncementTrend","2.5 %"])
}
...
UpperCIA.fun <- function(model){
  ...
  ifelse(sum(names(coef(model.object)) %in% "I(AnnouncementIndicatorTrend^2)")==1,
         ...
         confint(model.object)["AnnouncementTrend","97.5 %"])
}
...
ValueAILevel.fun <- function(model){
  ...
  est <- unname(coef(model)["SDILAnnouncement"] + coef(model)["SDILImplementation"])
  #VARIANCE
  se <- sqrt(
    vcov(model)["SDILAnnouncement","SDILAnnouncement"] +
      vcov(model)["SDILImplementation","SDILImplementation"] +
      (vcov(model)["SDILAnnouncement","SDILImplementation"] * 2)
  )
...
ValueAITrend.fun <- function(model){
  ...
  if(sum(names(coef(model.object)) %in% "I(AnnouncementIndicatorTrend^2)")==1) {
    ...
    est <- unname(coef(model.object)["I(AnnouncementIndicatorTrend^2)"] + coef(model.object)["ImplementationTrend"])
    se <- sqrt(vcov(model.object)["I(AnnouncementIndicatorTrend^2)","I(AnnouncementIndicatorTrend^2)"] +
                 vcov(model.object)["ImplementationTrend","ImplementationTrend"] +
                 (vcov(model.object)["I(AnnouncementIndicatorTrend^2)","ImplementationTrend"] * 2))
    LowerCI <- est + (-1 * qnorm(0.975)) * se
    UpperCI <- est + ( 1 * qnorm(0.975)) * se
    return(list(est,LowerCI,UpperCI));
  } else{
    est <- unname(coef(model.object)["AnnouncementTrend"] + coef(model.object)["ImplementationTrend"])
    se <- sqrt(vcov(model.object)["AnnouncementTrend","AnnouncementTrend"] +
                 vcov(model.object)["ImplementationTrend","ImplementationTrend"] +
                 (vcov(model.object)["AnnouncementTrend","ImplementationTrend"] * 2))
    LowerCI <- est + (-1 * qnorm(0.975)) * se
    UpperCI <- est + ( 1 * qnorm(0.975)) * se
    return(list(est,LowerCI,UpperCI))
  }
...
LevelTrendTable <- function(model,n=1, m=1000){
  df <- setNames(data.frame(
    #Announcement
    Announcement.Level=summary(eval(parse(text=paste0(model))))$tTable["SDILAnnouncement","Value"]*m,
    Announcement.Level.LowerCI=confint(eval(parse(text=paste0(model))))["SDILAnnouncement","2.5 %"]*m,
    Announcement.Level.UpperCI=confint(eval(parse(text=paste0(model))))["SDILAnnouncement","97.5 %"]*m,
    ...
    #Implementation
    Implementation.Level=summary(eval(parse(text=paste0(model))))$tTable["SDILImplementation",c("Value")]*m,
    Implementation.Level.LowerCI=confint(eval(parse(text=paste0(model))))["SDILImplementation","2.5 %"]*m,
    Implementation.Level.UpperCI=confint(eval(parse(text=paste0(model))))["SDILImplementation","97.5 %"]*m,
    Implementation.Trend=summary(eval(parse(text=paste0(model))))$tTable["ImplementationTrend",c("Value")]*m,
    Implementation.Trend.LowerCI=confint(eval(parse(text=paste0(model))))["ImplementationTrend","2.5 %"]*m,
    Implementation.Trend.UpperCI=confint(eval(parse(text=paste0(model))))["ImplementationTrend","97.5 %"]*m,
...
###############################################################################
#TABLE 5a. LEVEL AND TREND CHANGES: VOLUME
###############################################################################

Table5a <- data.frame(lapply(rbind(
  ...
  "ConfectioneryVolume" =LevelTrendTable("Confectionery")),round,3))

###############################################################################
#TABLE 5b. LEVEL AND TREND CHANGES: SUGAR
###############################################################################

Table5b <- data.frame(lapply(rbind(
  ...
  "ConfectionerySugar" =LevelTrendTable(model="ConfectionerySugar", n=1000, m=1)),round,9))

###############################################################################
#TABLE 5c. CHANGES BETWEEN COUNTERFACTUAL: VOLUME
###############################################################################

Table5c <- data.frame(lapply(rbind(
...
"ConfectioneryVolume" =PrePostTable("Confectionery")),round,8))

###############################################################################
#TABLE 5d. CHANGES BETWEEN COUNTERFACTUAL: SUGAR
#THE FINAL POSITIVE VALUE FOR LOWER TIER SUGAR OCCURED IN WEEK 253
###############################################################################

Table5d <- data.frame(lapply(rbind(
  ...
  "ConfectionerySugar" =PrePostTable(model="ConfectionerySugar", n=1000, m=1)),round,5))
...
doc <- read_docx() %>% 
       ...
       FormatLevelTrendTable.fun(Categories=c("HigherTierVolume","LowerTierVolume","NoLevyVolume","LowSugarVolume","NoSugarVolume","BottledWaterVolume",
                                      "AlcoholVolume","MilkBasedVolume","NASFruitJuiceVolume","PowderVolume","ConfectioneryVolume"),Tab=Table5a))

doc <- body_add_par(doc, value="") #CREATE GAP BETWEEN TABLES

doc <- body_add_table(doc, #TABLE 5b
                      FormatLevelTrendTable.fun(Categories=c("HigherTierSugar","LowerTierSugar","NoLevySugar","LowSugarSugar",
                                                             "MilkBasedSugar","NASFruitJuiceSugar","PowderSugar","ConfectionerySugar"),Tab=Table5b))
                      
doc <- body_add_par(doc, value="") #CREATE GAP BETWEEN TABLES

doc <- body_add_table(doc, #TABLE 5c
                      FormatPrePostTable.fun(Categories=c("HigherTierVolume","LowerTierVolume","NoLevyVolume","LowSugarVolume","NoSugarVolume","BottledWaterVolume",
                                                          "AlcoholVolume","MilkBasedVolume","NASFruitJuiceVolume","PowderVolume","ConfectioneryVolume"),Tab=Table5c))

doc <- body_add_par(doc, value="") #CREATE GAP BETWEEN TABLES

doc <- body_add_table(doc, #TABLE 5d
       FormatPrePostTable.fun(Categories=c("HigherTierSugar","LowerTierSugar","NoLevySugar","LowSugarSugar",
                                "MilkBasedSugar","NASFruitJuiceSugar","PowderSugar","ConfectionerySugar"),Tab=Table5d))

print(doc, target = "/dph/sdil/KWP221/Analysis/RegressionTable/RegressionTablesUncontrolled.docx") #EXPORT TABLES
