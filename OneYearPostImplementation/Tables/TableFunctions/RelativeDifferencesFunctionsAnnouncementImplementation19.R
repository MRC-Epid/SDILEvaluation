...
RelativeDifferencesAI.fun <- function(model,n=1, TimePointI1=264, TimePointI2=264){
...
list("AnnouncementImplementation.pcDiff"=est*100, 
     "AnnouncementImplementation.pcDiff.LowerCI"=(est + (-1.96 * se)) * 100,
     "AnnouncementImplementation.pcDiff.UpperCI"=(est + ( 1.96 * se)) * 100)
}
...
RelativeDifferencesQuadAI.fun <- function(model,n=1, TimePointI1=264, TimePointI2=264){
...
RelativeDifferencesEasterAI.fun <- function(model,n=1, TimePointI1=264, TimePointI2=264){
...
Without <- 
  sprintf("x1+x2*%f+x3*%f+x4*%f+x9*%f+x10*%f+x11*%f+x16*%f+x17*%f", 
          eval(parse(text=paste0("Data$",covariates[2])))[TimePointI2],
          eval(parse(text=paste0("Data$",covariates[3])))[TimePointI2],
          eval(parse(text=paste0("Data$",covariates[4])))[TimePointI2],
#           eval(parse(text=paste0("Data$",covariates[5])))[TimePointI2],
...
#MODELS WILL ONLY HAVE AN ANNOUNCEMENT INDICATOR TREND
###############################################################################

RelativeChangeAnnouncementImplementation.fun <-function(model,n=1,TimePointI1=264, TimePointI2=264){
  model.object = eval(parse(text=paste0(model)))
  ifelse(sum(names(coef(model.object)) %in% "I(AnnouncementIndicatorTrend^2)")==1,
         list(RelativeDifferencesQuadAI.fun(model,n, TimePointI1, TimePointI2)),
         ifelse(sum(row.names(data.frame(unlist(coefficients(model.object)))) %in% "Easter")==1,
                list(RelativeDifferencesEasterAI.fun(model,n, TimePointI1, TimePointI2)),
                list(RelativeDifferencesAI.fun(model,n, TimePointI1, TimePointI2))))
}
