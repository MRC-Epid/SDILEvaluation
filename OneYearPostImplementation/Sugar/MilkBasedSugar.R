###############################################################################
#MILK BASED ITS REGRESSION
#DAVID PELL
#08/11/19 
#UPDATED 12/03/2020 TO INCLUDE ALL MILK CATEGORIES
###############################################################################
#OPEN DATA
###############################################################################
...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("MilkBased","Toiletries")]
...
# 
# #ASSUMING THERE IS NO LEVEL CHANGE THE END POINT IS VERY SIMILAR WHEN 
# #I(ImplementationIndicatorTrend^2) + I(ImplementationIndicatorTrend^3)
# #IS INCLUDED AND WHEN IT IS NOT SO FOR SIMPLICITY EXCLUDE IT.
...
MilkBasedSugar <- gls(Model, data=SDIL, correlation=corARMA(p=4, q=4, form=~Time|Indicator),method="ML")
