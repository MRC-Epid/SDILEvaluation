...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("NASFruitJuice","Toiletries")]
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=7,form=~Time|Indicator)))
...
# anova(ARIMA,update(ARIMA,correlation=corARMA(p=5, q=5,form=~Time|Indicator))) #COEFFICIENT MATRIX ERROR
...
NASFruitJuiceSugar <- gls(Model, data=SDIL, correlation=corARMA(p=25, form=~Time|Indicator),method="ML")
