...
SDIL <- WeeklyVolume[WeeklyVolume$SDIL2 %in% c("NoLevy","Toiletries")]
...
# summary(update(model_ols, ~.+ I(AnnouncementIndicatorTrend^2) + I(AnnouncementIndicatorTrend^3))) #YES - BUT DIFFERENT TO PREVIOUS ANALYSIS
...
# 
# model_ols <- summary(update(model_ols, ~. +I(AnnouncementIndicatorTrend^2)))
...
NoLevySugar <- gls(Model, data=SDIL, correlation=corARMA(p=4,form=~Time|Indicator),method="ML")
